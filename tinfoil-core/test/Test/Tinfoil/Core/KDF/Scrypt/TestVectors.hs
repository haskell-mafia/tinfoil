...
module Test.Tinfoil.Core.KDF.Scrypt.TestVectors where
...
import           Tinfoil.Core.Data (Credential(..), Entropy(..))
import           Tinfoil.Core.KDF.Scrypt.Internal

import           Test.QuickCheck


-- Test vectors from the scrypt paper (Percival C., Stronger Key Derivation
-- via Sequential Memory-Hard Functions, 2009).

data TestVector =
  TestVector {
      testCredential :: Credential
    , testSalt       :: Entropy
    , testParams     :: ScryptParams
    , testHash       :: ByteString
  } deriving (Eq, Show)

instance Arbitrary TestVector where
  arbitrary = elements testVectors
...
testVectors = [
  ...
  , TestVector (Credential "password")
               (Entropy "NaCl")
               (ScryptParams 10 8 16)
               (BS.pack [
  ...
  , TestVector (Credential "pleaseletmein")
